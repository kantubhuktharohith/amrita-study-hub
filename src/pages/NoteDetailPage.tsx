...
import { ArrowLeft, Download, Calendar, User, FileText, Flag } from "lucide-react";
  ...
  if (!note) {
    return (
      <div className="container py-16 text-center">
        ...
        <Link to="/browse">
          <Button variant="ghost" className="mt-4">Back to Browse</Button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="container max-w-3xl py-8">
      ...
      <div className="rounded-lg border bg-card p-6 shadow-card">
        ...
        {note.description && (
          <p className="mb-6 text-sm text-foreground/80 leading-relaxed">{note.description}</p>
        )}
        ...
        <div className="mb-6 flex flex-wrap gap-4 text-sm text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <User className="h-4 w-4" /> {note.uploadedBy}
          </span>
          <span className="flex items-center gap-1.5">
            <Calendar className="h-4 w-4" /> {new Date(note.uploadDate).toLocaleDateString()}
          </span>
          <span className="flex items-center gap-1.5">
            <Download className="h-4 w-4" /> {note.downloads} downloads
          </span>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <Button
            variant="outline"
            onClick={() => toast.info("Report submitted. An admin will review it.")}
          >
